...
export default async function handler(req, res) {
    ...
    if (isNaN(teacherId)) {
        return res.status(400).json({ error: 'Bad Request', message: 'ID ครูผู้สอนไม่ถูกต้อง' })
    }
    ...
    try {
        if (req.method === 'DELETE') {
            ...
            return res.status(200).json({ message: 'ลบครูผู้สอนสำเร็จ', id: teacherId })
        }
        ...
        if (error.code === 'P2025') {
            return res.status(404).json({ error: 'Not Found', message: 'ไม่พบครูผู้สอนที่ต้องการลบ' })
        }
        // P2003: Foreign key constraint failed (e.g., trying to delete a teacher who has a schedule)
        if (error.code === 'P2003') {
            return res.status(409).json({ error: 'Conflict', message: 'ไม่สามารถลบครูผู้สอนนี้ได้ เนื่องจากยังมีตารางเรียนเชื่อมโยงอยู่' })
        }

...
export default function CreateStudent() {
    ...
    useEffect(() => {
        ...
        const fetchGroups = async () => {
            try {
                ...
                if (!res.ok) {
                    throw new Error(data.message || 'Failed to fetch class groups.')
                }
                setGroups(data)
                setLoading(false)
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                const data = await res.json()
                throw new Error(data.message || 'เกิดข้อผิดพลาดในการสร้างนักเรียน')
            }

            // Show success alert and redirect
            setAlertType('success')
            setAlertMessage('✅ สร้างนักเรียนใหม่สำเร็จ! กำลังนำทางกลับ...')
            setTimeout(() => {
